 ...
 * @copyright Copyright (c) 2024 seffeng
 */
namespace Seffeng\LaravelHelpers\Middlewares;
 ...
 *
 * 1、App\Http\Kernel，增加中间件
 * protected $routeMiddleware = [
 *     ...
 *     'throttle.action' => \Seffeng\LaravelHelpers\Middlewares\ThrottleRequests::class,
 *     ...
 * ];
 * 2、路由增加中间件配置，10秒内2次
 * Route::group(['middleware' => ['throttle.action:2,10,,1'], function() {
 * });
 */
class ThrottleRequests
{
    ...
    /**
     * limiter keys 增加参数[用于重复提交限制]
     *
     * @author zxf
     * @date   2024-06-12
     * @var boolean
     */
    protected static $shoudParamKeys = true;
     ...
     */
    public static function with($maxAttempts = 60, $decayMinutes = 1, $prefix = '', $unit = 2)
    {
     ...
     * @param  \Closure  $next
     * @param  integer|string  $maxAttempts
     * @param  float|integer  $decayMinutes
     * @param  string  $prefix
     * @param  integer  $unit
     ...
     */
    public function handle($request, Closure $next, $maxAttempts = 60, $decayMinutes = 1, $prefix = '', $unit = 2)
    {
        $this->setUnit($unit);
    ...
    protected function handleRequest($request, Closure $next, array $limits)
    {
        foreach ($limits as $limit) {
            ...
            $this->limiter->hit($limit->key, $this->getDecayTime($limit->decayMinutes));
        }
    ...
    {
        $key = self::$shoudParamKeys ? ('|' . $request->method() . '|' . $request->path() . '|' . (http_build_query($request->all()))) : '';
        if ($user = $request->user()) {
            return $this->formatIdentifier($user->getAuthIdentifier() . $key);
        } elseif ($route = $request->route()) {
            return $this->formatIdentifier($route->getDomain() . '|' . $request->ip() . $key);
        }
     ...
     * @param  integer  $maxAttempts
     * @param  integer  $remainingAttempts
     * @param  integer|null  $retryAfter
     * @return \Symfony\Component\HttpFoundation\Response
     */
    ...
    {
        $this->unit = in_array($unit, [self::UNIT_SEC, self::UNIT_MIN]) ? $unit : self::UNIT_MIN;
    }
     ...
     * @return integer
     */
    public function getDecayTime(int $decayMinutes)
    {
        return $this->getUnitIsSec() ? $decayMinutes : $decayMinutes * 60;
    }
     ...
     * @return boolean
     */
    public function getUnitIsSec()
    {
        return $this->unit === self::UNIT_SEC;
    }
     ...
     * @author zxf
     * @date   2024-06-12
     * @param boolean $shoudParamKeys
     * @return void
     */
    public static function shoudParamKeys(bool $shoudParamKeys = true)
    {
        self::$shoudParamKeys = $shoudParamKeys;
    }
}
